...
void simular(vector<vector<char> > cuadricula, int paso) {
    ...
    cout << "Fila con más incendios: " << fila_maxima + 1 << endl;
    ...
    for (int i = 0; i < M; ++i) {
        ...
        while (!valida) {
            ...
            for (char c : fila) {
                if (c != 'T' && c != 'F' && c != '.' && c != 'X') {
                    valida = false;
                    cout << "Caracter inválido '" << c << "'. Caracteres válidos: T, F, ., X. Intente de nuevo." << endl;
                    break;
                }
            }
        }
...
void modificar_bosque(vector<vector<char> >& bosque) {
    ...
    cout << "Ingrese el nuevo carácter (T, F, ., X): ";
    cin >> nuevo;
...
int main() {
    ...
    while (true) {
        ...
        switch (opcion) {
            ...
            case 2:
                if (M <= 0 || N <= 0) {
                    cout << "Primero ingrese las dimensiones (Opción 1)." << endl;
                    break;
                }
            ...
            case 3:
                if (bosque.empty()) {
                    cout << "Primero ingrese el estado del bosque (Opción 2)." << endl;
                    break;
                }
                simular(bosque, 0);
                break;
